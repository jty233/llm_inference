...
public:
    ...
    Tensor(std::vector<T> data, std::vector<int> shape) :  data(std::move(data)), shape(std::move(shape)), device(TensorDevice::CPU) {
        initShape();
    }

    Tensor() : device(TensorDevice::CPU) {}

    Tensor(Tensor&& oth) :  data(std::move(oth.data)), shape(std::move(oth.shape)), jump(std::move(oth.jump)), device(oth.device) {
    }

    Tensor(const Tensor& oth) : data(oth.data), shape(oth.shape), jump(oth.jump), device(oth.device) {
    }
    ...
    void load(std::vector<T> data, std::vector<int> shape, TensorDevice dev = TensorDevice::CPU) {
    ...
    void asShape(std::vector<int> shape, TensorDevice dev = TensorDevice::CPU) {
        this->shape = std::move(shape);
        this->device = dev;
        int size = 1;
    ...
    // 设置设备
    void setDevice(TensorDevice dev) {
        device = dev;
    }
    ...
    void toGPU() {
#ifdef __CUDACC__
        if (device == TensorDevice::CPU && !data.empty()) {
            // 分配GPU内存
            gpu_ptr = GPUManager::getInstance().allocate(data.size() * sizeof(T));
            // 复制数据到GPU
            GPUManager::getInstance().copyHostToDevice(gpu_ptr, data.data(), data.size() * sizeof(T));
    ...
    void toCPU() {
#ifdef __CUDACC__
        if (device == TensorDevice::GPU && gpu_ptr != nullptr && !data.empty()) {
            // 分配CPU内存并从GPU复制数据
            data.resize(size());  // 确保CPU数据有正确的大小
            GPUManager::getInstance().copyDeviceToHost(data.data(), gpu_ptr, data.size() * sizeof(T));
...
private:
    ...
    ~Tensor() {
        if (gpu_ptr) {
#ifdef __CUDACC__
            GPUManager::getInstance().free(gpu_ptr);
#endif
            gpu_ptr = nullptr;
        }
    }
